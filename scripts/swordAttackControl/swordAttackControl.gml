// Sword input
var sword_input = keyboard_check_pressed(ord("K")) || gamepad_button_check_pressed(global.pad_num, gp_face2);
...
if sword_input != 0 && sword_can_attack {
	...
	with (instance_create_layer(x, y, "Instances", obj_sword)) {
		...
		// criar no obj_sword a colisão com os inimigos e nao aqui?
		// when enemies are implemented, change their state to dead
		//if place_meeting(x, y, enemies_parent) {
		//    inst = instance_place(x, y, enemies_parent)
		//    if inst != noone {
		//        with (inst) {
		//            alive = false
		//        }
